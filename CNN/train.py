"""
train.py — Two-stage fine-tuning of EfficientNet-B3 on MTSD.

Stage 1 (epochs 1–5):
  Backbone frozen. Only the classification head trains.
  Fast convergence without disrupting pretrained features.

Stage 2 (epochs 6–30):
  All layers unfreeze with differential LR:
    backbone LR = LR_BACKBONE (low, gentle)
    head LR     = LR_HEAD     (higher)
  Cosine LR schedule with linear warm-up.

Usage:
  python train.py                              # local MTSD in project root
  python train.py --mtsd-root /path/to/mtsd   # custom root
  python train.py --epochs 40 --batch-size 64 --use-hf   # HF mirror
...
def seed_everything(seed: int = config.SEED):
    ...
    # benchmark=True lets cuDNN auto-tune convolution kernels for the fixed
    # input size — meaningful speedup on GPU. Incompatible with deterministic
    # mode, so we only enable one at a time.
...
class MixupCrossEntropy(nn.Module):
    def __init__(self, label_smoothing: float = config.LABEL_SMOOTHING):
        super().__init__()
        self.ce = nn.CrossEntropyLoss(label_smoothing=label_smoothing)

    def forward(self, logits, target):
        if isinstance(target, tuple):
            # Mixup: target = (y_a, y_b, lam)
            y_a, y_b, lam = target
...
def accuracy(logits: torch.Tensor, labels: torch.Tensor, topk=(1, 5)):
    with torch.no_grad():
        maxk = max(topk)
        batch = labels.size(0)
...
def train_one_epoch(
    ...
    for step, (images, labels) in enumerate(loader):
        ...
        if isinstance(labels, (list, tuple)) and len(labels) == 3:
            labels = (
                labels[0].to(device, non_blocking=True),
                labels[1].to(device, non_blocking=True),
                labels[2],
            )
            plain_labels = labels[0]   # for accuracy calc
        else:
        ...
        # Metrics (use plain labels for accuracy even with mixup)
        ...
        if step % 50 == 0:
            ...
            print(
                ...
                f"loss={loss_m.avg:.4f} | top1={acc1_m.avg:.4f} | top5={acc5_m.avg:.4f} | "
                f"{elapsed:.1f}s"
            )
...
def train(args):
    ...
    # ── Data ──────────────────────────────────────────────────────────────────
    train_ds, val_ds, num_classes, label_map = get_datasets(
        root   = args.mtsd_root,
        use_hf = args.use_hf,
    )
    ...
    persistent  = config.NUM_WORKERS > 0   # keep worker processes alive between epochs
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # STAGE 2: unfreeze all, differential learning rates
    # ─────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-hf",     action="store_true",
                        help="Use HuggingFace dataset mirror instead of local MTSD")
